...
registerShape("line", {
  ...
  render(g, obj, isSelected) {
    ...
    // Line goes from (0,0) to (width, height) within bounding box
    const line = document.createElementNS(ns, "line");
    line.setAttribute("x1", "0");
    line.setAttribute("y1", "0");
    line.setAttribute("x2", String(obj.width));
    line.setAttribute("y2", String(obj.height));
    line.setAttribute("stroke", resolveStroke(obj.style.strokeColor));
    line.setAttribute("stroke-width", String(Math.max(obj.style.strokeWidth, 2)));
    line.setAttribute("stroke-opacity", String(obj.style.strokeOpacity));
    ...
    hitLine.setAttribute("x2", String(obj.width));
    hitLine.setAttribute("y2", String(obj.height));
    hitLine.setAttribute("stroke", "transparent");
    hitLine.setAttribute("stroke-width", "12");
    hitLine.classList.add("draw-hit-area");
...
registerShape("textbox", {
  ...
  render(g, obj, isSelected) {
    ...
    // Transparent background rect (for hit testing and bounding)
    const rect = document.createElementNS(ns, "rect");
    rect.setAttribute("width", String(obj.width));
    rect.setAttribute("height", String(obj.height));
    rect.setAttribute("fill", resolveFill(obj.style.fillColor));
